...
public interface Command {
    ...
    public companion object CommandRegistry : Registry<Command> {
        ...
        override fun register(value: Command) {
            ...
            val root = CommandNodeRoot(id, value)
            Root.nodes.add(root)
            registerBukkitCommand(root)
        }

        context(Plugin)
        private fun registerBukkitCommand(root: CommandNodeRoot) {
            val bukkitCommandMap: Field = Bukkit.getServer().javaClass.getDeclaredField("commandMap")
            bukkitCommandMap.isAccessible = true
            val commandMap: CommandMap = bukkitCommandMap.get(Bukkit.getServer()) as CommandMap
            root.command.literals.forEach {
                commandMap.register(id, object : org.bukkit.command.Command(it) {
                    override fun execute(sender: CommandSender, commandLabel: String, args: Array<out String>): Boolean {
                        return root.run(args.toList()).isSuccess
                    }
                })
            }
        }
    ...
    public fun resolveNextNode(tokens: List<String>): Pair<Command, Int>? =
        nodes.fold<Command, Pair<Command, Int>?>(null) { best, it ->
            ...
            if (result == Int.MAX_VALUE) return Pair(it, 1) // Root name-spaced command
...
@PublishedApi
internal open class CommandNodeLiteral @PublishedApi internal constructor(
...
internal class CommandNodeRoot(
    ...
    override fun matches(tokens: List<String>): Int =
        if (command.literals.any { "$namespace:$it" == tokens.first() }) Int.MAX_VALUE
        else if (command.literals.any { it == tokens.first() }) 1
        else -1
